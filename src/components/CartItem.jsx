...
import { formatPrice, generateAmount } from '../utils/Index'
...
const handleAmount=(e)=>{
    ...
    <article key={cartID} className='mb-12 flex flex-col gap-y-4 sm:flex-row flex-wrap border-b border-base-300 pb-6 last:border-b-0'>
        ...
        <p className='mt-8 text-sm capitalize flex items-center gap-x-2'>
        ...
        <div className='sm:ml-12'>
            <div className="form-control max-w-xs">
                <label htmlFor="amount" className='label p-0'>
                    ...
                    <select name="amount" id="amount" className='m-2 select select-base select-bordered select-xs' value={amount} onChange={handleAmount}>
                        {generateAmount(amount + 5)}
                    </select>
                </label>
            </div>
            <button className='mt-2 link-primary link-hover text-sm' onClick={removeItemFromCart}>Remove</button>
        </div>
